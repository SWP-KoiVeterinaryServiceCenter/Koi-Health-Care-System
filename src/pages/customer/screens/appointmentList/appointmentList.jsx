...
import { confirmAppointmentsThunk, missAppointmentsThunk } from "../../../../store/apiThunk/appointment";
...
const AppointmentList = (props) => {
  ...
  const accounts = useSelector(currentappointmentSelector);
  ...
  // Filter the accounts to only show "Finished" ones
  useEffect(() => {
    const finishedAccounts = accounts?.filter(account => account.status === "Finished");

    setFilteredRows(
      finishedAccounts?.map((account, index) => ({
        ...account,
      ...
      .then((response) => {
        ...
        if (response.error) {
          ...
          Swal.fire("Success", "Appointment confirmed successfully!", "success").then(() => {
            dispatch(getCurrentUserAppointmentsThunk()); // Reload data after success
          });
        }
      })
      .catch((error) => {
  ...
  const handlePrescription = (rowId) => {
    navigate(`/customer/prescriptionList`, {
      state: { id: rowId },
    });
  };
  ...
  const columns = [
    ...
    },
    {
      ...
      flex: 1,
      renderCell: ({ row: { price } }) => <div>{price}</div>,
    },
    {
      field: "actions",
      headerName: "Actions",
